...
### YOUR CODE HERE
REGULARIZATION = [0,0.00001,0.00005,0.0001,0.0005,0.001,0.005,0.01,0.05,0.1,0.25]
### END YOUR CODE
...
for regularization in REGULARIZATION:
    ...
    weights = sgd(lambda weights: softmax_wrapper(trainFeatures, trainLabels, 
        weights, regularization), weights, 3.0, 15000, PRINT_EVERY=100)

    # Test on train set
    _, _, pred = softmaxRegression(trainFeatures, trainLabels, weights,regularization)
    trainAccuracy = accuracy(trainLabels, pred)
    print "Train accuracy (%%): %f" % trainAccuracy

    # Test on dev set
    _, _, pred = softmaxRegression(devFeatures, devLabels, weights,regularization)
...
_, _, pred = softmaxRegression(testFeatures, testLabels, BEST_WEIGHTS,BEST_REGULARIZATION)
